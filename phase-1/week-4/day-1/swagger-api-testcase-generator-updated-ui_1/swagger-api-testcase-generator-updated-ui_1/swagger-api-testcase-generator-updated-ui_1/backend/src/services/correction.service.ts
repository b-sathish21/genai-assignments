...
import { CorrectionOutputSchema } from "../schemas/llm.schemas.js";
...
async function applyLlmCorrection(specification: JsonObject, errors: ValidationIssue[]) {
  ...
  return CorrectionOutputSchema.parse(raw);
}
...
export async function correctSpecification(
  ...
  if (!safeOnly && !report.isValid && isLlmEnabled()) {
    const llm = await applyLlmCorrection(active, report.errors.length ? report.errors : validationErrors);
    active = llm.correctedSpecification;
    changes = [...changes, ...llm.changes];
    source = deterministic.changes.length ? "deterministic+inception" : "inception";
